...
if sel=="All":
    ...
    # build wide table: pick price col heuristically
    series_map = {}
    for name, df in datasets:
        try:
            df2 = ensure_dt_index(df)
        except Exception:
            continue
        price_col = next((c for c in ['close','price','mid','last'] if c in df2.columns), None)
        if price_col is None:
            numeric = [c for c in df2.columns if pd.api.types.is_numeric_dtype(df2[c])]
            if numeric:
                price_col = numeric[0]
            else:
                continue
        s = df2[price_col].resample(freq).last().ffill()
        series_map[f"{name}__{price_col}"] = s
    if not series_map:
        st.error("No numeric series found")
        st.stop()
    wide = pd.concat(series_map.values(), axis=1); wide.columns = list(series_map.keys())
    st.subheader("Combined wide series preview"); st.dataframe(wide.head())
else:
